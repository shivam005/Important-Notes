...
### How gateway works? 
It is used to handle all non-functional requirements and to route the traffic to the respective microservice on the basis of the path being passed in the URI. Earlier, zuul and other gateways were used but now netflix has stopped further enhancement hence it is better to go with the spring cloud gateway which is the better way of doing it. We register spring cloud gateway as eurake client and gets registered with the eureka server. We just need to pass the name of the gateway and the url that we mention in the application.properties would internally start routing the request to the respective service. 

### Saga design Pattern
It is used in case of ditributed transactions. If any of the transaction got failed during processing the chain of transaction then we have got to reverse the previous txn as well, hence in case of SAGA, two flows are executed 1). is happy flow and 2). unhappy flow. In case of unhappy flow, we trigger the list of compensating txn. This can be done using two ways 1). Choreography based saga 2). Orchastrator based sage. In case of choreographer, a MQ is used and all the status is pushed to the MQ which is further picked up the subscriber and the required actions are taken but when the txns becames complex then it is not considered good as there is no proper tracking mechanism. Hence, the better option is to use Orchastrator based saga wherein an orchastrator interacts with all the service and takes the decision like reversing the transaction etc.

### What is Command (Write Operations) Query (Read Operations) Responsibility Segregation (CQRS) design pattern 
It is about segragating the read and write operations. For reading operations, we can prefer to go with the DB which has faster read capabilities and to write, we can use different db. Our responsibility would be to just keep all the DB in sync. 
